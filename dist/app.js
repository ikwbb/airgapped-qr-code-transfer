...
let receiver = new Receiver(), stream = null, scanning = false, cameraPending = false;
...
let lastAcceptedAt = 0, wakeLock = null, displayModules = [];
const scanCanvas = document.createElement('canvas');
const scanContext = scanCanvas.getContext('2d', { willReadFrequently: true });
// Two well-sized codes use a wide display; a narrow display starts with one.
if ($('send-stage').clientWidth >= 680) $('code-count').value = '2';

async function syncWakeLock() {
  if (!navigator.wakeLock) return;
  if (running || scanning) {
    if (document.visibilityState !== 'visible' || wakeLock) return;
    try { wakeLock = await navigator.wakeLock.request('screen'); wakeLock.addEventListener('release', () => { wakeLock = null; }); } catch { /* Optional browser capability. */ }
...
function selectMode(next) {
  ...
  if (next === mode) return;
  if (next === 'receive') pauseSend(); else stopCamera();
  mode = next;
...
$('code-count').onchange = () => {
  ...
  if (transfer) { fillQueue(); updateCeiling(); if (resume) startSend(); }
};
...
function fillQueue() {
  ...
  if (!encodeWorker) {
    ...
    encodeWorker.onmessage = ({ data }) => {
      if (data.generation !== encodeGeneration) return;
      encodePending--;
  ...
  while (displayQueue.length + encodePending < 6) {
    ...
    for (let i = 0; i < Number($('code-count').value); i++) {
      ...
      labels.push(item[0] === 0 ? 'File info' : item[0] === 1 ? `Block ${item[1] + 1}` : `Repair ${item[1] + 1}`);
    }
    encodePending++;
...
function startSend() {
  ...
  nextDisplayAt = 0; displayedAt = []; fillQueue();
...
async function setupDecoder() {
  ...
  if (detector) { write('decoder-label', 'QR'); return; }
  createDecodeWorker();
  write('decoder-label', 'QR');
}
function createDecodeWorker() {
  decodeWorker = new Worker('./decode-worker.js', { type: 'module' });
  decodeWorker.onmessage = ({ data }) => {
    if (data.generation !== cameraGeneration) return;
    decodeBusy = false;
    if (data.error) {
...
async function startCamera() {
  if (cameraPending || scanning || receiver.verified || finishPending) return;
  ...
  cameraPending = true; $('camera-start').disabled = true;
  ...
  try {
    ...
    if (generation !== cameraGeneration || mode !== 'receive') { candidate.getTracks().forEach(t => t.stop()); return; }
    stream = candidate;
    ...
    await setupDecoder();
    if (generation !== cameraGeneration) { decodeWorker?.terminate(); decodeWorker = null; return; }
    ...
    stream.getVideoTracks()[0].onended = () => {
      if (scanning) { stopCamera(); message('receive-message', 'The camera disconnected. Your progress is kept; reconnect it and resume.', true); }
    };
    scanning = true; decodeBusy = false; lastVideoTime = -1;
    ...
    // stopCamera can invalidate generation after an error or a tab change.
    cameraPending = false; $('camera-start').disabled = receiver.verified || finishPending;
  }
}
function stopCamera() {
  scanning = false; cameraGeneration++; cancelAnimationFrame(animation); decodeBusy = false;
...
$('camera-start').onclick = () => {
  if (scanning) { stopCamera(); message('receive-message', 'Camera paused. Your progress is kept.'); }
  else startCamera();
};
...
function scan(now) {
  ...
  // Preserve detail for high-density codes, but cap fallback work per camera frame.
  ...
  if (detector) {
    detector.detect(scanCanvas).then(codes => {
      ...
      for (const code of codes) if (code.rawValue.startsWith('B1:')) acceptFrame(code.rawValue);
    }).catch(() => {
      if (generation !== cameraGeneration) return;
...
function acceptFrame(text) {
  if (finishPending || receiver.verified || !text.startsWith('B1:')) return;
  const before = receiver.received;
  const result = receiver.accept(text);
  if (result === 'rejected') { write('receive-note', 'A damaged frame was ignored. The sender will repeat it.'); return; }
  ...
  if (result === 'waiting') { write('camera-status', 'SIGNAL FOUND'); message('receive-message', 'Signal found. Waiting for file information, which repeats automatically.'); return; }
  if (receiver.received > before || result === 'manifest') {
...
function updateReceiver() {
  const r = receiver, ratio = r.count ? Math.round(100 * r.received / r.count) : 0;
  ...
  write('received-count', r.count ? `${r.received.toLocaleString()} / ${r.count.toLocaleString()} blocks` : '0 blocks collected');
...
async function completeReceive() {
  ...
  try {
    ...
    write('receive-note', `SHA-256 verified · ${format(completed.meta.size)} · ${completed.recovered} blocks recovered with parity`);
...
$('clear-receive').onclick = () => {
  if (receiver.received && !receiver.verified && !confirm('Clear the blocks collected for this file?')) return;
  resetReceiver();
};
...
document.addEventListener('visibilitychange', () => {
  if (document.hidden) {
    ...
    if (scanning) { stopCamera(); message('receive-message', 'Camera paused while this tab was hidden. Resume to continue.'); }
  } else syncWakeLock();
});
window.addEventListener('pagehide', () => { pauseSend(); stopCamera(); if (downloadUrl) URL.revokeObjectURL(downloadUrl); });
...
if ('serviceWorker' in navigator && isSecureContext) {
  navigator.serviceWorker.register('./sw.js').then(async registration => {
    ...
    const cache = await caches.open('beam-v2');
    if (await cache.match('./index.html')) write('offline-state', 'Ready to reopen offline');
